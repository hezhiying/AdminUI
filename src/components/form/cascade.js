const Cascade = function ($cascade, cascadeData) {
  this.target = $cascade;
  ...
  this.cacheData = [];
  let selects = $cascade.data('cascade').split(',');
  this.selects = []; //{"name":"province", "elm":$("[name=province]")}
  let self = this;
  //遍历初始化
  selects.forEach((key, index) => {
    let $elm = key.startsWith('#') || key.startsWith('.') ? $(key) : $("select[name='" + key + "']");
...
//初始化数据
Cascade.prototype.setData = function (data) {
...
//初始化数据
Cascade.prototype.initData = function (sIndex = 0) {
  let self = this;
  if (this.cascadeData) {
    //==========有数据直接渲染
    ...
    //==========通过ajax一次性获取数据
    $.ajax({url: this.target.data('cascadeData'), dataType: this.ajax.dataType, method: this.ajax.method}).then(function (data) {
...
//填充数据
Cascade.prototype.render = function (selectIndex) {
  //初始化select列表
...
$.fn.uiCascade = function (data) {
  $(this).each((i, e) => {
    ...
    $this.data('cascadeObj', new Cascade($this));
  });
};

export default {
  onload: () => {
    $('[data-cascade]').uiCascade();
  },
  event: (elm) => {
    $(elm).find('[data-cascade]').uiCascade();
  }
}